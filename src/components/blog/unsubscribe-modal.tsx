...
export default function UnsubscribeModal({ token }: { token?: string }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-hidden bg-zinc-950/80 p-2 backdrop-blur-md sm:p-4">
    <section role="dialog" aria-modal="true" aria-labelledby="unsubscribe-title" className="relative max-h-[calc(100dvh-1rem)] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-background shadow-2xl shadow-black/50 sm:rounded-[28px]">
      ...
      <div className="border-b bg-gradient-to-br from-violet-500/15 via-background to-sky-500/10 p-6 text-center sm:p-10">
        ...
        <h1 id="unsubscribe-title" className="mt-5 text-2xl font-black tracking-tight sm:text-3xl">{result ? "Salida confirmada" : "¿Quieres salir de la comunidad?"}</h1>
